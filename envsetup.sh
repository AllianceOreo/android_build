...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^alliance_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $ALLIANCE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ALLIANCE_BUILD=
    fi
...
function print_lunch_menu()
{
    ...
    if [ "z${ALLIANCE_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the ALLIANCE GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/alliance/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/alliance/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function repopick() {
    T=$(gettop)
    $T/vendor/alliance/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $ALLIANCE_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.alliance.device | grep -q "$ALLIANCE_BUILD");
    then
        ...
        echo "The connected device does not appear to be $ALLIANCE_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.alliance.device | grep -q "$ALLIANCE_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/alliance/build/envsetup.sh
